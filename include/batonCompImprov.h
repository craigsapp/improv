...
// Last Modified: Sun Oct  1 16:49:22 PDT 2000 (converted from batonImprov.h)
// Filename:      ...sig/code/control/improv/batonCompImprov.h
// Web Address:   http://sig.sapp.org/include/sig/batonCompImprov.h
// Syntax:        C++
//
...
int    chooseComputerInputPort(void);
int    chooseComputerOutputPort(void);
...
Synthesizer computer;       // MIDI connection to another computer
...
int runImprovInterface(void) {
   ...
   while (1) {
      ...
      computer.processIncomingMessages();
   ...
   // case 'H': break;
      case 'I':                         // set input port for Radio Drum
         ...
         cout << "radio drum input is now set to: "
              << baton.getInputPort() << endl;
         break;
      case 'J':                         // set the other computer input port
         cout << "other computer MIDI input is currently set to device: " 
              << computer.getInputPort() << endl;
         computer.pause();
         computer.setInputPort(chooseComputerInputPort());
         computer.openInput();
         cout << "Computer input is now set to: "
              << computer.getInputPort() << endl;
         break;
   ...
   // case 'N': break;
	  ...
	  case 'T':                         // set MIDI out device for synth
         ...
         cout << "synthesizer MIDI output is now set to: "
              << synth.getPort() << endl;
         break;
      case 'U':                             // set other computer MIDI out port
         cout << "other computer MIDI output is currently set to device: "
              << computer.getOutputPort() << endl;
         computer.setOutputPort(chooseComputerOutputPort());
         computer.openOutput();
         cout << "other computer MIDI output is now set to: "
              << computer.getOutputPort() << endl;
         break;
   // case 'V': break;
   // case 'W': break;
...
//////////////////////////////
//
// chooseComputerInputPort -- have the user select the 
//    MIDI input device to communicate with the other computer.
//

int chooseComputerInputPort(void) {
   computer.pause();
   int portCount = computer.getNumInputPorts();
   ...
   cout << "Choose one of the following MIDI input ports "
        << "for other computer communication:" << endl;
   displayInputPorts();
...
//////////////////////////////
//
// chooseComputerOutputPort -- have the user select the 
//    MIDI output device to communicate with the other computer.
//

int chooseComputerOutputPort(void) {
   computer.pause();
   int portCount = computer.getNumOutputPorts();
...
int readmidiconfig(void) {
   int batonin, batonout, synthout, computerin, computerout;
   ...
   while (!infile.eof()) {
      ...
      } else if (strcmp(token, "computerin:") == 0) {
         token = strtok(NULL, " \t");
         if (token == NULL) {
            cout << "Warning: error in midi.def at computerin:" << endl;
            computerin = -1;
         } else {
            sscanf(token, "%d", &computerin);
         }
      } else if (strcmp(token, "computerout:") == 0) {
         token = strtok(NULL, " \t");
         if (token == NULL) {
            cout << "Warning: error in midi.def at computerout:" << endl;
            computerout = -1;
         } else {
            sscanf(token, "%d", &computerout);
         }
      }
      infile.getline(buffer, 512, '\n');
   }
   ...
   if (computerin < -1) {
      computerin = chooseComputerInputPort();
   }
   if (computerout < -1) {
      computerout = chooseComputerOutputPort();
   }
   ...
   computer.pause();
   computer.setInputPort(computerin);
   computer.openInput();
   computer.unpause();

   computer.setOutputPort(computerout);
   computer.openOutput();
   ...
   cout << "Synthesizer output set to device " << synthout << " = "
      << synth.getName() << endl;
   cout << "Other Computer MIDI input set to device " << computerin << " = "
      << computer.MidiInput::getName() << endl;
   cout << "Other Computer MIDI output set to device " << computerout << " = "
      << computer.MidiOutput::getName() << endl;
...
// md5sum: a2c7a0fb3f09486ef61ec045e24bd5d0 batonCompImprov.h [20050403]
