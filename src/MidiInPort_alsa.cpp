...
// use the following header for version of ALSA newer or equal to 0.9:
...
CircularBuffer<smf::MidiEvent>** MidiInPort_alsa::midiBuffer = NULL;
...
MidiInPort_alsa::~MidiInPort_alsa() {
   ...
   if (objectCount == 0) {
      ...
      cerr << "Error: bad MidiInPort_alsa object count!: " 
           << objectCount << endl;
      exit(1);
   }
}
...
void MidiInPort_alsa::extract(smf::MidiEvent& event) {
   if (getPort() == -1) {
      smf::MidiEvent temp;
      event = temp;
      return;
...
void MidiInPort_alsa::insert(const smf::MidiEvent& aMessage) {
   if (getPort() == -1)   return;
...
smf::MidiEvent& MidiInPort_alsa::message(int index) {
   if (getPort() == -1) {
...
void *interpretMidiInputStreamPrivateALSA(void * arg) {
   ...
   message      = new smf::MidiEvent[MidiInPort_alsa::numDevices];
...
// md5sum: 5a0ada3ea7d5a68c19bdf9451fe4b49c MidiInPort_alsa.cpp [20050403]
