...
ostream*  MidiInPort_oss::tracedisplay                   = &cout;
...
MidiInPort_oss::~MidiInPort_oss() {
   ...
   if (objectCount == 0) {
      ...
      cerr << "Error: bad MidiInPort_oss object count!: " 
           << objectCount << endl;
      exit(1);
   }
}
...
void *interpretMidiInputStreamPrivate(void *) {
   ...
   static int count = 0;
   if (count != 0) {
      cerr << "Cannot run this function more than once" << endl;
      exit(1);
   } else {
...
/*
      cout << "packet bytes "
           << " 0x" << hex << (int)packet[0]   // packet type
           << " 0x" << hex << (int)packet[1]   // possible MIDI byte
           << " 0x" << hex << (int)packet[2]   // device number
           << " 0x" << hex << (int)packet[3]   // unused for MIDI bytes
           << endl;
*/
...
/*
            cout << "MIDI byte: " << (int)packet[1] << endl;
*/
...
// md5sum: 025a56fa11e6617982da06ca4405bc1e MidiInPort_oss.cpp [20050403]
